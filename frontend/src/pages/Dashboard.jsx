...
const Dashboard = (CUId) => {
  ...
  const getExamNames = async () => {
    const { data } = await axios.get(`/api/v1/user/${CUId.CUId}`);
    console.log(CUId.CUId);
    setExamNameStorage(data.quiz);
    console.log(data);
    console.log(setExamNameStorage);
    setIsLoading(false);
  }

  const deleteExam = (id) => {
    axios.delete(`/api/v1/${id}`).then((response) => {
  ...
  const handleName = (e) => {
    ...
    if (examName == "") {
      ...
      console.log(newExam)
      axios.post("/api/v1/quiz/new", newExam).then((response) => {
        console.log(response.status);
        console.log(response.data);
      });
      setDummy(dummy + 1)
                ...
                examNameStorage.map((name) => (
                  ...
                  >
                    <TableCell component="th" scope="row" onClick={() => { navigator.clipboard.writeText("http://localhost:3000/quiz/" + name._id) }}>
                      <span> {name.examname} </span>
                    </TableCell>
                    <TableCell align="right"><Link to={`/analyze/${name._id}`}><Button><BarChart style={{ verticalAlign: "middle", padding: "5px" }} />Details</Button></Link></TableCell>
