...
const Question = ({
  ...
  const handleCreatorUser = async () => {
    const { data } = await axios.get('/api/v1/quizzes/' + id.id)
    console.log(data)
    setPass(data.quiz.creatorUserId == userId)
    setIsLoading(false)
  }
  ...
  const handleSubmit = (e) => {
    ...
    if (pass == userId) {
      ...
      axios.patch(`/api/v1/userquiz/add/${examId}`, userExam).then((response) => {
    ...
    if (pass == userId) {
      ...
      const userOptions = {
        examReview: {
          qAnswers: i,
          qCorrect: correct,
          qTitle: questions.question[currQues].questionTitle,
        }
      };
      console.log(userOptions)
      axios.put(`/api/v1/userquiz/add/${id.id}`, userOptions).then((response) => {
  ...
  return (
    ...
    <Container>
      ...
      <SingleQuestion>
        ...
        <h2>{questions.question[currQues].questionTitle}</h2>
        <Options>
